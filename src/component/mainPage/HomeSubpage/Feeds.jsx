...
const Feeds = () => {
  ...
  const email = organizationDetails.organizationDetails.companyMail;

  
  const handlePostAnnouncement = async () => {
    try {
      const response = await axios.post(
        "https://employe-tango-backend.onrender.com/tango/api/feed",
        { message, email }
      );
  ...
  const getDataFeed = async () => {
    try {
      const response = await axios.get(`https://employe-tango-backend.onrender.com/tango/feeds/${email}`);
  ...
  return (
    <div className="h-screen overflow-scroll mb-[180vh] md:w-[130%] ">
      <button
